...
class CatalogSpec extends AnyFunSpec with Matchers {
  ...
  describe("CatalogSpec") {
    ...
    it("Create LC8 Layer Collection") {
      val layerUS = StacCollection(
        stacVersion = "0.9.0",
        stacExtensions = List("eo", "view", "https://example.com/stac/landsat-extension/1.0/schema.json"),
        id = "layer-us",
        title = Some("Landsat 8 L1"),
      ...
      val layerCA = layerUS.copy(
        id = "layer-ca",
        properties = layerUS.properties.deepMerge(Map("layer" -> "layer-ca".asJson).asJsonObject)
      )

      layerUS.asJson.deepDropNullValues shouldBe getJson("/catalogs/landsat8-layers/collection-layer-us-v1.json")
      layerUS.asJson.as[StacCollection].valueOr(throw _) shouldBe layerUS

      layerCA.asJson.deepDropNullValues shouldBe getJson("/catalogs/landsat8-layers/collection-layer-ca-v1.json")
      layerCA.asJson.as[StacCollection].valueOr(throw _) shouldBe layerCA
    }

    it("Create LC8 Layer Item") {
      val collection = getJson("/catalogs/landsat8/collection-v1.json").as[StacCollection].valueOr(throw _)
      val layerUS    = getJson("/catalogs/landsat8-layers/collection-layer-us-v1.json").as[StacCollection].valueOr(throw _)
      val layerCA    = getJson("/catalogs/landsat8-layers/collection-layer-ca-v1.json").as[StacCollection].valueOr(throw _)
      ...
      val item = StacItem(
        ...
        properties = Map(
          // layer extension
          "layers"                               -> List(layerUS.id, layerCA.id).asJson,
          "collection"                           -> "landsat-8-l1".asJson,
          "datetime"                             -> "2014-06-02T09:22:02Z".asJson,
          "eo:gsd"                               -> 15.asJson,
          "eo:cloud_cover"                       -> 10.asJson,
      ...
      item.asJson.deepDropNullValues shouldBe getJson("/catalogs/landsat8-layers/item-layer-v1.json")
      item.asJson.as[StacItem].valueOr(throw _) shouldBe item
    }
  }
}
